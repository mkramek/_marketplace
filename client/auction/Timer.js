...
export default function Timer(props) {
    ...
    return (<div className={classes.subheading}>
        {!timeLeft.timeEnd ? <Typography component="p" variant="h6" >
            ...
            {timeLeft.seconds != 0 && `${timeLeft.seconds} s`} left <span className={classes.endTime}>{`(kończy się ${new Date(props.endTime).toLocaleString()})`}</span></Typography> :
            <Typography component="p" variant="h6" >Aukcja zakończona</Typography>}
    </div>
    )
}