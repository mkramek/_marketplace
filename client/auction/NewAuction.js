...
export default function NewAuction() {
    ...
    const clickSubmit = () => {
        if (values.bidEnd < values.bidStart) {
            setValues({ ...values, error: "Aukcja nie może zakończyć sie przed rozpoczęciem" })
        }
        else {
            ...
            values.minPrice && auctionData.append('minPrice', values.minPrice)
            create({
                userId: jwt.user._id
            }, {
                t: jwt.token
    ...
    return (<div>
        <Card className={classes.card}>
            <CardContent>
                ...
                <TextField id="name" label="Nazwa Przedmiotu" className={classes.textField} value={values.itemName} onChange={handleChange('itemName')} margin="normal" /><br />
                <TextField
                    ...
                    margin="normal"
                /><br />
                <TextField id="startingBid" label="Początkowa Cena (zł)" className={classes.textField} value={values.startingBid} onChange={handleChange('startingBid')} margin="normal" /><br />
                <br />
                <TextField
                    id="datetime-local"
                    label="Data rozpoczęcia aukcji"
                    type="datetime-local"
                ...
                <TextField
                    id="min-price"
                    label="Cena minimalna (opcjonalnie)"
                    type="number"
                    step="0,01"
                    defaultValue={0.00}
                    className={classes.textField}
            ...
            <CardActions>
                ...
                <Link to='/myauctions' className={classes.submit}><Button variant="contained">Anuluj</Button></Link>
            </CardActions>
        </Card>
    </div>)
}
