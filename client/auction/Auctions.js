...
export default function Auctions(props) {
    ...
    const auctionState = (auction) => {
        return (
            <span>
                {currentDate < new Date(auction.bidStart) && `Aukcja zaczeła sie ${new Date(auction.bidStart).toLocaleString()}`}
                {currentDate > new Date(auction.bidStart) && currentDate < new Date(auction.bidEnd) && <>{`Aukcja jest bieżąca | ${auction.bids.length} oferty |`} {showTimeLeft(new Date(auction.bidEnd))}</>}
                {currentDate > new Date(auction.bidEnd) && `Aukcja zakończona | ${auction.bids.length} bids `}
