...
const userByID = async (req, res, next, id) => {
  try {
    ...
    return res.status(400).json({
      error: "Nie można otrzymać danych użytkownika"
    })
  }
};
...
const getResetPasswordLink = async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      user.password_token = uuid5(new Date().toISOString(), "8ec52ed2-08a2-5a13-91b0-fcfd83430e02");
      await user.save();
...
const resetPassword = async (req, res) => {
  try {
    ...
    return res.status(400).json({ error: 'Błąd przetwarzania zapytania' });
  }
};
...
const isSeller = (req, res, next) => {
  ...
  if (!isSeller) {
    return res.status('403').json({
      error: "Użytkownik nie jest sprzedawcą"
    })
  }
  next()
};
    ...
    myStripe.customers.update(req.profile.stripe_customer, {
      ...
      if (err) {
        return res.status(400).send({
          error: "Nie mozna umieścic detali zamówienia"
        })
      }
...
const createCharge = (req, res, next) => {
  if (!req.profile.stripe_seller) {
    return res.status('400').json({
      error: "Połącz swoje konto płatności w Stripe"
    })
  }
